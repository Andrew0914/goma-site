...
export default function TheNavbar() {
  ...
  const navbarClasses = classNames(styles.thenavbar, {
    [styles.thenavbar_top]: isAtTop,
  });
    ...
    <>
      ...
      <nav className={navbarClasses} ref={navbarRef}>
        <div className={`${styles.thenavbar_content} container`}>
          <Tooltip
            title={getCurrentFestivity().detail}
          ...
          >
            <h4 className="heading--4">
              <a href="/">
                {t("title", { emoji: getCurrentFestivity().icon })}
              </a>
            </h4>
          </Tooltip>
              ...
              {routes.map((route, index) => (
                <li key={index}>
                  <Link
                    href={route.path}
                    className={
                      pathname === route.path ? styles.routeActive : ""
                    }
                  >
                    {route.name}
                  </Link>
